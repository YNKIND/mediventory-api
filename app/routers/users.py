...
def create_user(
    ...
    if payload.role == "owner" and current_user.role != "owner":
        raise HTTPException(
            ...
            detail="Only an owner can create another owner",
        )
    ...
    if payload.password:
        ...
        if not payload.send_invite:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Set a password or send an invite, otherwise this account cannot be used",
            )
        # Unusable placeholder. The invite link is the only way in.
    ...
    if payload.role is not None and payload.role != user.role:
        ...
        if user.role == "owner":
            ...
            if remaining_owners == 0:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="There must always be at least one active owner",
                )
