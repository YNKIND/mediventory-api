...
def resolve_clinic_ids(db: Session, current_user: models.User, scope: str, single_clinic_id: int) -> list[int]:
    """Which clinics to include. 'all' = every clinic the user is owner/admin of.
    Anything else = just the currently selected clinic."""
    if scope == "all":
...
def cost_per_procedure(
    ...
    for key, count in proc_count.items():
        ...
        rows.append({
            "procedure_id": 0,  # aggregated by name, not a single id
